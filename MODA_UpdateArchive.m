...
function [Archive_X_updated, Archive_F_updated, Archive_member_no]=MODA_UpdateArchive(Archive_X, Archive_F, Particles_X, Particles_F, Archive_member_no)
...
for i=1:size(Archive_F_temp,1)
    ...
    for j=1:i-1
        if any(Archive_F_temp(i,:) ~= Archive_F_temp(j,:))
            if MODA_dominates(Archive_F_temp(i,:),Archive_F_temp(j,:))
                o(j)=1;
            elseif MODA_dominates(Archive_F_temp(j,:),Archive_F_temp(i,:))
                o(i)=1;
                break;
            end
        else
